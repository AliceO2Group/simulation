...
All modules that are not detectors are also called **"passivel modules"**.

By default, `o2-sim` runs a simulation with the complete Run3 geometry. Modules can be excluded from the geometry using the `--skipModules <mod1> [<mod2> [...]]`. In that case they do not contribute to the overall material budget in the simulation. If only certain modules should be included, one uses `-m|--modules <mod1> [<mod2> [...]]`. Note that `--skipModules` takes precednce over `-m`.

By default, all detectors produce hits and are therfore considered read-out. The detectors' readout can be switched off with `--skipReadoutDetectors <det1> [<det2> []]`. Reading out only a certain subset of detectors is achieved with `-readoutDetectors <det1> [<det2> [...]]`. Note that `--skipReadoutDetectors` takes precedence over `--readoutDetectors`. No matter what the readout options are:
...
The detector names are [centrally defined](https://github.com/AliceO2Group/AliceO2/blob/dev/DataFormats/Detectors/Common/include/DetectorsCommonDataFormats/DetID.h). Any of those can be passed to the `--redoutDetectors` and `-m` options (as well as their corresonding `--skip` counter part).
...
* `COMP`: the compensator in very forwward region,
* `SHIL`: the shield to filter muons.

The modules for the ALICE Run5 develompments are descried [here](run5.md).
